...
export default function PostJobPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      ...
      setFileSize(`${(selected.size / 1024).toFixed(2)} KB`);
    }
  };
  ...
  const handlePostJob = async () => {
    if (!jobTitle || !file) {
      alert('Please provide job title and file.');
      return;
    }
    ...
    try {
      ...
      const result = await postJob(jobTitle, file);
      console.log('Upload success:', result);
      setIsSuccess(true);

      setTimeout(() => {
        window.location.href = '/employer'; // redirect
      }, 3000);
    } catch (error) {
      console.error(error);
      ...
      {isSuccess ? (
        ...
        >
          ...
          <div className="mt-4 flex justify-end gap-3">
            ...
            <Button
              type="submit"
              disabled={!jobTitle || !file || isSubmitting || isUploading}
